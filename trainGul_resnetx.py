# -*- coding: utf-8 -*-
"""
Created on Mon Sep  9 15:52:54 2024
...
import modelResNeXt_SE_SA
import data_utilsTest, data_utilsTrain
...
def train(args):
    ...
    lfcc_model=modelResNeXt_SE_SA.ResNeXt(num_blocks=[2, 2, 2, 2], cardinality=4, bottleneck_width=64).to(args.device)
    ...
    training_set = data_utilsTrain.ASVDataset(is_train=True, is_logical=is_logical, transform=transforms2,feature_name=args.features)
    trainDataLoader = DataLoader(   training_set, batch_size=args.batch_size, shuffle=True)
   
    validation_set = data_utilsTrain.ASVDataset(is_train=False, is_logical=is_logical, transform=transforms2, feature_name=args.features, is_eval=args.is_eval, eval_part=args.eval_part)
    valDataLoader = DataLoader(validation_set, batch_size=args.batch_size, shuffle=True)
    ...
    for epoch_num in tqdm(range(args.num_epochs)):
        ...
        for i,(lfcc, labels,batch_meta)  in enumerate((trainDataLoader)):
        ...
        with torch.no_grad():
            ...
            print("val")
            for i,(lfcc, labels,batch_meta)  in enumerate((valDataLoader)):
                lfcc = lfcc.unsqueeze(1).float().to(args.device)
                labels = labels.to(args.device)
