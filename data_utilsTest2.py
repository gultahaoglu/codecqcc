# -*- coding: utf-8 -*-
"""
Created on Wed Sep 11 15:35:42 2024
...
class ASVDataset(Dataset):
    ...
    def __init__(self,transform=None,is_train=True,sample_size=None,is_logical=True,feature_name=None, is_eval=False,eval_part=0):
        ...
        self.files_dir = os.path.join(self.data_root, '{}_{}'.format(self.prefix, self.dset_name )+v1_suffix, 'flac')
        self.protocols_fname = os.path.join(self.protocols_dir,'ASVspoof2019.{}.cm.{}.txt'.format(track, self.protocols_fname))
        self.cache_fname = 'cache_{}{}_{}_{}.npy'.format(self.dset_name,'_part{}'.format(eval_part) if is_eval else '',track, feature_name)
        self.cache_matlab_fname = 'cache_{}{}_{}_{}.mat'.format(self.dset_name, '_part{}'.format(eval_part) if is_eval else '',track, feature_name)
        ...
        if os.path.exists(self.cache_fname):
            ...
            if os.path.exists(self.cache_matlab_fname):
                ...
                self.data_x, self.data_y, self.tag = self.read_matlab_cache(self.cache_matlab_fname)
                self.files_meta = self.parse_protocols_file(self.protocols_fname)
                print('Dataset loaded from matlab cache ', self.cache_matlab_fname)
                torch.save((self.data_x, self.data_y, self.data_sysid,self.tag, self.files_meta),
    ...
    def read_matlab_cache(self, filepath):
        ...
        return data_x.astype(np.float32), data_y.astype(np.int64), tag
